...
func TestBotCommand_Reload(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantErr:  false,
			wantMsg:  "So, if my body gets killed, big whoop! I just download into another body. I'm immortal, baby!",
			wantType: "shutdown",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := bc.Reload(tt.args.event); (err != nil) != tt.wantErr {
			...
			if r.Message != tt.wantMsg {
				t.Errorf("Reload() message = %v, want %v", r.Message, tt.wantMsg)
			}
			if r.Type != tt.wantType {
				t.Errorf("Reload() type = %v, want %v", r.Type, tt.wantType)
			}
			if r.UserId != "testuser123" {
				t.Errorf("Reload() userId = %v, want %v", r.UserId, "testuser123")
			}
		})
	}
}

func TestBotCommandHelp_Reload(t *testing.T) {
	type args struct {
		request BotCommand
	}
	tests := []struct {
		...
		{
			name: "test reload help",
			args: args{
				request: BotCommand{},
			},
			wantHelp: "Causes the bot to shutdown, pull any changes from git, and restart",
			wantDesc: "Reloads the bot",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h := BotCommandHelp{}
			response := h.Reload(tt.args.request)
			if response.Help != tt.wantHelp {
				t.Errorf("Reload() help = %v, want %v", response.Help, tt.wantHelp)
			}
			if response.Description != tt.wantDesc {
				t.Errorf("Reload() description = %v, want %v", response.Description, tt.wantDesc)
			}
		})
	}
}