...
func (mm MagicMirror) Cast(wizard *wavinghands.Wizard, target *wavinghands.Living) (string, error) {
    if (len(wizard.Right.Sequence) >= len(mm.Sequence) && strings.HasSuffix(wizard.Right.Sequence, mm.Sequence)) ||
        (len(wizard.Left.Sequence) >= len(mm.Sequence) && strings.HasSuffix(wizard.Left.Sequence, mm.ShSequence)) {
