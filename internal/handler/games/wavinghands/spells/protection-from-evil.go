...
func (pfe ProtectionFromEvil) Cast(wizard *wavinghands.Wizard, target *wavinghands.Living) (string, error) {
	rightMatch := len(wizard.Right.Sequence) >= len(pfe.Sequence) &&
		strings.HasSuffix(wizard.Right.Sequence, pfe.Sequence)
	leftMatch := len(wizard.Left.Sequence) >= len(pfe.Sequence) &&
		strings.HasSuffix(wizard.Left.Sequence, pfe.Sequence)
